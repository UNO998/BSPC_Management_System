...
<head>
    ...
    if (isset($_GET['type'])) {
        ...
        if ($type == 'employee') $query = "SELECT * FROM employee";
        else $query = "SELECT * FROM employee AS e, " . $type . " AS a WHERE a.empID=e.empID";

        if (isset($_GET['sort'])) $query = $query . " ORDER BY ". $_GET['sort'] ." ". $_GET['order'];
        ...
        foreach ($table[0] as $name) {
            echo '<td>' . htmlspecialchars($name) .
                '<i class="up" onclick="sort(\''. $type .'\', \''. $name .'\', \'ASC\')"></i> <br>
                <i class="down" onclick="sort(\''. $type .'\', \''. $name .'\', \'DESC\')"></i></td>';
        }
        echo '<td> </td>';
        echo '</tr>';
    ...
    <script>
        ...
        function sort(type, name, order) {
            var url = "Table.php?type="+type +"&sort="+ name +"&order="+ order;
